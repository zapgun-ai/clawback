...
This file lists **every** option at its default value, except `host` and `tls`,
set for a LAN-reachable bind. That bind also requires `adminToken`, which is
left as a **null placeholder** for you to set (see the security note).
...
- **selfSign** (default `false`) — when `tls` is on and no cert/key exists at the
  ...
  one-shot generator.)
- **adminToken** (default `null`; left as a placeholder here — set it) — shared-secret bearer required on
